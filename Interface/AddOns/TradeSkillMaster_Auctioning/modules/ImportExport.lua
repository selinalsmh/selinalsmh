...
function TSM:Encode(groupName)
	...
	for itemString in pairs(TSM.db.profile.groups[groupName]) do
		if not strfind(itemString, "battlepet:") then
			tItem = itemString
...
function TSM:OpenImportFrame()
	...
	btn:SetCallback("OnClick", function()
			...
			for groupName, items in pairs(TSM.db.profile.groups) do
				local ignoreRandomEnchants = TSM.Config:GetBoolConfigValue(groupName, "ignoreRandomEnchant", true)
				for itemString in pairs(items) do
					if ignoreRandomEnchants then
						itemString = TSMAPI:GetSafeItemID(itemString)
					end
					itemIDs[itemString] = groupname
				end
			end
			...
			for i, v in pairs(importData) do
				if i == "items" then
					for _, itemString in pairs(v) do
						if not TSM.itemReverseLookup[itemString] and not itemIDs[TSMAPI:GetSafeItemID(itemString)] then
							TSM.db.profile.groups[groupName][itemString] = true
						elseif not excludeExisting then
							local itemIDGroup = itemIDs[TSMAPI:GetSafeItemID(itemString)]
							if itemIDGroup then
								for iString in pairs(TSM.db.profile.groups[itemIDGroup]) do
									if TSMAPI:GetSafeItemID(iString) == TSMAPI:GetSafeItemID(itemString) then
										TSM.db.profile.groups[itemIDGroup][iString] = nil
									end
								end
							else
				...
				elseif i ~= "version" and i ~= "itemIDs" and i ~= "itemIDGroups" then
