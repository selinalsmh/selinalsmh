...
local rt_1, rt_mask = COMBATLOG_OBJECT_RAIDTARGET1, COMBATLOG_OBJECT_RAIDTARGET_MASK
...
local function colour(name)
	if (instance == "raid" and FatalityDB.output1 ~= "SELF") or (instance == "party" and FatalityDB.output2 ~= "SELF") then
		return strip(name)
...
local function icon(flag)
	...
	return number and (((instance == "raid" and FatalityDB.output1 == "SELF") or (instance == "party" and FatalityDB.output2 == "SELF")) and format(rt_path, number) or format(rt, number)) or ""
end
...
local function send(message)
	if instance == "raid" then
		if FatalityDB.output1 == "SELF" then
			...
			SendChatMessage(message, FatalityDB.output1, nil, raid_output)
		end
	elseif instance == "party" then
		if FatalityDB.output2 == "SELF" then
			print(message)
		else
			SendChatMessage(message, FatalityDB.output2, nil, party_output)
		end
	end
end
...
function Fatality:FormatOutput(guid)
	...
	if msg:len() > 255 and ((instance == "raid" and self.db.output1 ~= "SELF") or (instance == "party" and self.db.output2 ~= "SELF")) then
...
function Fatality:COMBAT_LOG_EVENT_UNFILTERED(timestamp, event, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, destGUID, destName, destFlags, destRaidFlags, ...)
	...
	elseif event == "SPELL_ENERGIZE" then
		spellID, _, school = ...
		if spellID == 31818 then amount, overkill = floor(UnitHealthMax(srcName) * 0.15), -1 end
...
function Fatality:RegisterEvents()
	...
	raid_output = (self.db.output1 == "CHANNEL" and GetChannelName(self.db.channel1)) or (self.db.output1 == "WHISPER" and self.db.whisper1)
	party_output = (self.db.output2 == "CHANNEL" and GetChannelName(self.db.channel2)) or (self.db.output2 == "WHISPER" and self.db.whisper2)
end
