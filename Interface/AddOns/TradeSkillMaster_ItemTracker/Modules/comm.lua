...
function Comm:SendInventoryData(target, shouldReturnData)
	local data = {CopyTable(TSM.characters), CopyTable(TSM.guilds), shouldReturnData}
...
function Comm:OnCommReceived(_, msg, _, sender)
	...
	if isValid then
		...
		for name, guildData in pairs(data[2]) do
			...
			if not TSM.guilds[name] then
				TSM.guilds[name] = guildData
			elseif (TSM.guilds[name].lastUpdate or 0) < (guildData.lastUpdate or 0) then
				TSM.guilds[name] = guildData
			end
		end
